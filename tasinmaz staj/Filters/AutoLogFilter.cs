...
namespace tasinmaz_staj.Filters
{
    public class AutoLogFilter : IAsyncActionFilter
    {
        ...
        {
            // Controller'daki metot çalışıp response dönene kadar bekle
            var executedContext = await next();

            var user = context.HttpContext.User;

            // Kullanıcı yetki doğrulaması yapmışsa log at
            if (user.Identity.IsAuthenticated)
            {
                ...
                // Sadece veri değiştiren (CRUD'un C, U, D) isteklerini logluyoruz
                if (method == "POST" || method == "PUT" || method == "DELETE")
                {
                    ...
                    var log = new Log
                    {
                        ...
                        Description = $"{controllerName} üzerinde {operation} işlemi gerçekleştirildi.",
